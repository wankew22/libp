...
class MoleculeDataset(InMemoryDataset):
    ...
    def _process_and_save(self):
        suppl = Chem.SDMolSupplier(self.sdf_file, removeHs=False, sanitize=False, strictParsing=False)
        data_list = []
        print("开始读取分子...")
        ...
        for mol in suppl:
            if mol is None or mol.GetNumConformers() == 0:
                continue
            try:
                if not mol.HasProp("y"):
                    continue
                cls = mol.GetProp("y").strip() # read y
                label = int(cls) # convert to int
        ...
        print(f"✅ 成功读取分子数量：{len(data_list)}") 
